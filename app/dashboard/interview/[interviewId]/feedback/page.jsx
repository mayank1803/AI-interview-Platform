    ...
    Find below interview questions with correct answers, your answers, and feedback for improvement
            ...
            {Object.entries(groupedFeedback).map(([question, answers], index) => (
                <Collapsible key={index} className='mt-7'>
                    <CollapsibleTrigger className='p-4 bg-white shadow-lg rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors duration-300 ease-in-out flex justify-between text-left gap-4 w-full'>
                        ...
                        <div className='flex flex-col gap-4'>
                            {answers.map((item, idx) => (
                                <div key={idx} className='p-4 border rounded-lg bg-gradient-to-r from-blue-50 to-white shadow-md hover:shadow-lg transition-shadow duration-300'>
                                    ...
                                    <h2 className='p-2 border rounded-lg bg-blue-50 text-sm text-blue-900 mt-2'><strong>Feedback: </strong>{item.feedback}</h2>
                                </div>
                            ))}
                        </div>
                    </CollapsibleContent>
